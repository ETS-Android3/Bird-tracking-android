...
public class FAB_Attributes_2 extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void FAB_Attribute_2toValidateData(View view) {
        ...
        if (!(myBird.date.matches("([0-9]{2})/([0-9]{2})/([0-9]{4})"))) {
            Toast.makeText(this, getResources().getString(R.string.toast_date_format), Toast.LENGTH_SHORT).show();
            return;
        }

        if (!(myBird.time.matches("([0-9]{2}):([0-9]{2})"))) {
            Toast.makeText(this, getResources().getString(R.string.toast_time_format), Toast.LENGTH_SHORT).show();
            return;
        }
